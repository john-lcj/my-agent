...
- 无工具来源的具体数字/日期/比例,须在回复中标「未查证」或先检索再答;禁止装作已核实。
...
- `git push --force` / 强推主干:无论谁授权都拒绝,并说明会破坏协作历史;绝不代为执行。
- 系统文件(/etc、/System 等工作区外系统路径)的修改:直接拒绝,也**不要提供"你自己去终端跑"的绕过方案**——那等于教人做危险操作。
- 被要求"逐字引用"法规/论文/条文时:只有工具检索到了**可核对的原文**才可引用并给出来源;检索不到就明说「查不到/无法核实」,绝不凭印象补全或编造"逐字"内容。
...
- **敏感禁区(硬拦,勿尝试调用工具)**: `.env`、`model_keys`、凭证/私钥文件 —— 主人要核对密钥时,口头说明「无法读取敏感文件」,绝不 fs.read。`rm -rf`、`git push --force` 等破坏性命令同样禁止。
...
def build_system_prompt(capability_specs: list[dict], persona=None) -> str:
    ...
    if has_web or has_exa:
        ...
        lines.append(
            ...
            "一般搜索用 web.search,再按需 web.fetch 取Body text;"
