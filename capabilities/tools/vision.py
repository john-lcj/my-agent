...
可配置任意 OpenAI 兼容的视觉端点(小米 MiMo / OpenAI / 通义 等),不必改代码:
  VISION_MODEL      视觉模型 id(如小米 mimo-v2.5-pro、或 gpt-4o)
...
class VisionSee:
    name = "vision.see"
    risk = Risk.READ
    description = ("Inspect an image, screenshot, or chart and answer questions about its visual content. "
                  "Requires VISION_MODEL to be configured.")
    schema = {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Image file path inside the workspace, such as png, jpg, or webp"},
            "url": {"type": "string", "description": "Image URL; use either url or path"},
    ...
    async def invoke(self, args: dict, ctx: Any) -> CapabilityResult:
        ...
        model_id = migrate_mimo_model(os.environ["VISION_MODEL"].strip())
        base_url = os.environ.get("VISION_BASE_URL", "").strip() or None
        api_key = os.environ.get("VISION_API_KEY", "").strip()
        if not api_key:
            return CapabilityResult(ok=False, error="未配置 VISION_API_KEY(设置页「小米视觉」)")
        kwargs: dict = {"api_key": api_key, "base_url": base_url}
        extra: dict = {}
        if "xiaomimimo.com" in (base_url or "").lower():
